...
- ``memory_app.plugins``         插件 SPI 抽象 + 注册表 + 工厂（设计文档 §2.7）
- ``memory_app.plugins_default`` 项目内置默认插件实现
- ``memory_app.config_center``   配置中心（A+B 嵌套类层级，§2.8.6.1）
- ``memory_app.routers``         FastAPI 路由（健康检查、Admin API、未来 ingest/retrieve...）
- ``memory_app.settings``        启动期不可变配置（从 ``config/bootstrap.yaml`` 加载，§2.8.6.2）
