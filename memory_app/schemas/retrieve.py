...
class RetrievalIntent(str, Enum):
    ...
    检索 引入 ``IntentClassifier`` SPI 后，``AUTO`` 由 LLM/规则自动判定；
    核心实现 / 写入热路径 阶段调用方可手动指定。
    """
...
class RetrievalConfig(BaseModel):
    """检索时的请求级临时覆盖（对应  五级覆盖中的 request 层）。

    所有字段都是 Optional —— 不传即沿用配置中心当前值。
     受白名单约束：并非所有参数都允许 request 层覆盖。
    """

    model_config = ConfigDict(extra="forbid")

    over_fetch_factor: int | None = None  # 各通道过取倍数（，常规 4 / 复杂 12）
    similarity_threshold: float | None = None  # 阈值过滤（，默认 0.55）
...
class MemoryHit(BaseModel):
    ...
    model_config = ConfigDict(extra="allow")  # 允许后续版本扩展字段
